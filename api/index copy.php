...
$app->get('/getaverage/', function () {
    ...
    try 
    {       
        $qry = "SELECT avg(tr.rating_val) as rating_avg, SUM(ti.duration) as duration FROM telecast_info ti LEFT JOIN telecast_ratings tr ON tr.net = ti.net AND tr.date_time =ti.date_time WHERE (ti.net = '".$net."' AND tr.rating_type='".$metric."' AND tr.data_stream='lsd'";//
...
$select = $select." FROM telecast_info ti ";
$where = $where."tr.rating_type='".$metric."' AND ti.date_time <= '".$starttime."' AND ti.date_time >= DATE_ADD('".$starttime."',INTERVAL ".$weeks." WEEK)) ";
$orderby = " ORDER BY ti.date_time DESC;";
...
$app->get('/getweeks/', function () {
    ...
    {   
        $db = getDB();
        $sth = $db->prepare("SELECT ti.date_time, WEEK(ti.date_time,1) as wk from telecast_info as ti GROUP BY WEEK(ti.date_time,1) ORDER BY ti.date_time DESC;");
        $sth->execute();
        $data = $sth->fetchAll(PDO::FETCH_ASSOC);//(PDO::FETCH_OBJ);
